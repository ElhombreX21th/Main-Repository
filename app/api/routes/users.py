...
def list_users(
    admin: User = Depends(require_roles(UserRole.admin)), db: Session = Depends(get_db)
):
    return list(db.scalars(select(User).where(User.organization_id == admin.organization_id)))
